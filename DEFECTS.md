...
## Gone, because the code that had them is gone

| # | Where | What | How it closed |
| --- | --- | --- | --- |
| 1 | `get_rslow_stretch`, `ruptime.c` | The high-side edge replication starts at index `count` rather than `count + offset`, so whenever the **low** side was padded — a hypocentre near a fault edge — the fault's own last rows and columns were overwritten with values from further in. On a 12×12 fault with the hypocentre at the top edge, the deepest three rows of slowness were replaced by values three rows shallower. | **By deletion.** The padding existed only because the expanding-square tracker needed the source clear of every edge. Factored sweeping removes the singularity analytically instead, so there is no near-source region, no grid growth, no replication and nothing to reproduce. `rupture/mod.rs` predicted this would happen, and it did |
